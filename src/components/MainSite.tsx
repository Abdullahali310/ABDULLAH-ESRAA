...
export function MainSite({ audioPlaying, toggleAudio, onAudioError }: { audioPlaying: boolean; toggleAudio: () => void; onAudioError?: () => void }) {
  ...
  useEffect(() => {
    // تسريع تأثير الكونفيتي في البداية
    const duration = 1500; 
  ...
  const triggerFireworks = () => {
    const duration = 3 * 1000; // تقليل وقت الألعاب النارية
  ...
  const triggerPetals = () => {
    const duration = 6 * 1000; // تقليل وقت تساقط الورود إلى النصف
    const animationEnd = Date.now() + duration;
    const randomInRange = (min: number, max: number) => Math.random() * (max - min) + min;
    ...
    const interval: any = setInterval(function() {
      ...
      confetti({
        ...
        ticks: 200, // تسريع اختفاء الورود قليلاً
        gravity: randomInRange(0.3, 0.5), 
        origin: { x: Math.random(), y: -0.1 }, 
        colors: ['#9B2226', '#AE2012', '#BB3E03', '#B28938'], 
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 1, ease: "easeInOut" }} // دخول أسرع
    ...
    >
      ...
      <section className="relative min-h-[100svh] flex flex-col items-center justify-center px-4 overflow-hidden">        
        <motion.div
          ...
          transition={{ duration: 0.8, delay: 0.1, ease: "easeOut" }} // أسرع بكثير
        ...
        >
          ...
          <h1 className="font-display text-5xl md:text-8xl lg:text-[9rem] tracking-wide mb-8 leading-[1.6]">
             ...
             <motion.span 
                  ...
                  transition={{ repeat: Infinity, repeatDelay: 1, duration: 0.6 }} // نبض قلب أسرع
      ...
      <section className="py-40 px-6 relative z-10">
        <motion.div 
          ...
          transition={{ duration: 0.8, ease: "easeOut" }} // أسرع عند التمرير
      ...
      <section className="py-32 px-6 relative z-10">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-12 justify-center">
          ...
          {/* Katb Ketab Card */}
          <motion.div
            initial={{ opacity: 0, filter: "blur(5px)", scale: 0.95, y: 20 }}
            whileInView={{ opacity: 1, filter: "blur(0px)", scale: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.6, ease: "easeOut" }} // دخول أسرع
            className="flex-1 bg-white/70 backdrop-blur-md border border-[#C5A059]/20 shadow-xl rounded-xl p-12 text-center flex flex-col items-center justify-center transition-all duration-300 hover:border-[#C5A059]/60 hover:shadow-2xl hover:bg-white hover:scale-[1.02]" // أسرع عند التحويم
          >
            ...
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="px-10 py-4 border border-[#C5A059]/40 rounded-full text-[#B28938] font-body text-lg hover:bg-[#FDFBF7] hover:border-[#B28938] transition-colors duration-300"
            >
              عرض الموقع
            </a>
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.6, delay: 0.1, ease: "easeOut" }} // دخول أسرع
            className="flex-1 bg-white/90 backdrop-blur-md border-2 border-[#B28938]/30 shadow-[0_8px_30px_rgb(178,137,56,0.15)] rounded-xl p-12 text-center flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:border-[#B28938]/60 hover:shadow-[0_8px_40px_rgb(178,137,56,0.25)] md:z-10" // أسرع عند التحويم
          >
            <motion.div 
              animate={{ opacity: [0.2, 0.5, 0.2] }} 
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} // وميض أسرع
              className="absolute inset-0 bg-gradient-to-br from-transparent via-[#C5A059]/15 to-transparent pointer-events-none" 
            />
            
            <motion.div
               animate={{ y: [0, -5, 0] }}
               transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} // طفو أسرع للتاج
               className="mb-4"
            >
            ...
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="relative overflow-hidden px-12 py-4 border border-[#B28938]/60 bg-[#B28938]/5 rounded-full text-[#B28938] font-body text-xl transition-all duration-300 group"
            >
              <span className="relative z-10 transition-colors duration-300 group-hover:text-white">عرض موقع القاعة</span>
              <div className="absolute inset-0 bg-[#B28938] scale-x-0 group-hover:scale-x-100 origin-right transition-transform duration-300 ease-out z-0" />
            </a>
          </motion.div>
      ...
      <footer className="relative min-h-[70svh] flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-gradient-to-t from-[#FDFBF7] via-[#FDFBF7] to-transparent">        
        <motion.div
          ...
          transition={{ duration: 1, ease: "easeOut" }} // ظهور أسرع للنهاية
